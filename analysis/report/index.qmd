...
date: "2025-01-01"
categories: [r, zzcollab-compendia, reproducibility]
...
NARRATIVE INPUTS
  ...
  [ ] Lessons Learned  — 4 conceptual + 4 technical + 4 gotcha bullets
...
ANALYSIS DELIVERABLES (produced by analysis/scripts/, loaded — never re-computed — in this .qmd)
  ...
  [ ] Dockerfile + Makefile                 — `make docker-build && make docker-render-qmd`
...
IMAGES (4 total — hero + 3 ambiance, all in media/images/)
  ...
  [ ] Ambiance image 3 (100% width)         — before Lessons Learned
...
Current: penguin-hero.png (penguin theme - replace with your topic!)
...
![Engaging hero image that introduces your topic visually](media/images/penguin-hero.png){.img-fluid width=80%}
...
**Disclaimer:** This learning process is documented
here. Errors spotted or better approaches are always
welcome.
...
![Atmospheric image to maintain visual engagement - replace with relevant scene](media/images/ambiance1.jpg){.img-fluid}
...
**Background:** Basic R and ggplot2 familiarity is
helpful but not required. Concepts are explained
as we proceed.
...
Before examining the code, it is worth clarifying
what [topic] actually means. [Simple, plain-language
explanation of the concept. Use an analogy if
helpful.] In practice, this means [concrete example
or application].
...
We have 32 cars with 11 variables. We now examine
...
Average fuel efficiency is 20.1 MPG with
considerable variation (SD = 6.0).

# Exploring the Data

<!-- TEMPLATE INSTRUCTION: RESULTS VISUALIZATION SECTION

This is where your pre-generated figures come in.
...
We visualize these patterns using pre-generated
figures from `analysis/scripts/03_generate_figures.R`:
...
Cars with fewer cylinders are consistently more
fuel-efficient.
...
![Visual break - another atmospheric image maintaining engagement](media/images/ambiance2.jpg){.img-fluid}
...
Weight has the strongest correlation with MPG
(r = -0.87). We visualize that relationship:
...
Heavier cars consistently get worse mileage, a
relationship consistent with basic mechanics.
...
We fit a simple linear model to quantify this
relationship:
...
(R² = 0.75).** This is a reasonably strong fit. For
every 1,000 lbs of weight, fuel efficiency decreases
by about 5.3 MPG (95% CI: [-6.5, -4.1]).
...
We make predictions to illustrate the model in
practice:
...
A 2,000 lb car yields approximately 30 MPG, while a
4,000 lb car yields only approximately 15 MPG.
...
Before trusting these results, we check model
assumptions:
...
**Diagnostic checks:** Two to three potential
outliers (>2.5 SD) were found. These merit
investigation but do not substantially affect the
overall model fit.
...
No major patterns appear in the residuals, though
a couple of potential outliers warrant
investigation.
...
1. **Do not extrapolate too far** - This model is
   valid for weights between 1.5-5.5 thousand lbs.
   Predicting outside that range is unreliable.

2. **Correlation is not causation** - Weight
   correlates with MPG, but there are confounding
   variables (engine size, aerodynamics, etc.).

3. **Check model assumptions** - Always plot
   residuals. A high R² does not guarantee the
   model is appropriate.

4. **Small sample size** - With only 32 cars,
   confidence intervals deserve careful attention.
...
![Concluding visual - tie back to topic theme](media/images/ambiance3.png){.img-fluid}
...
- Always check residual plots; R² alone is not sufficient.
...
If additional time were available, the following
...
This exploration confirms that vehicle weight is a
powerful predictor of fuel efficiency, accounting
for 75% of the variance. The model is simple but
effective, though the limitations noted above apply.

Working through this analysis clarified [specific
technical skill you gained]. Additional extensions
(more predictors, non-linear models, modern data)
would all be instructive next steps.

**For those attempting this themselves:**
- Begin with exploration before modeling.
- Plot residuals before trusting the fit.
- High R² alone does not validate a model.
- Report confidence intervals alongside point
  estimates.
...
<!-- TEMPLATE INSTRUCTION: SETUP AND TROUBLESHOOTING

Practical reference for readers reproducing your work from scratch, and for
you when rendering breaks. This section is operational, not narrative, so
keep it terse and command-focused.

STRUCTURE:
1. Starting a fresh zzcollab project (if the reader has nothing yet)
2. Entering the development environment
3. Common rendering problems and fixes
4. Validating dependencies before publishing

Adjust the commands below if your post's compendium diverges from the
zzcollab defaults.
-->

## Starting a New Project

If you are not already working inside a zzcollab compendium, scaffold one
with the `blog` archetype so `zzcollab` wires up `analysis/report/index.qmd`
plus the `index.qmd`/`data`/`figures`/`media` symlink topology
automatically:
...
Packages installed this way are captured automatically in `renv.lock` when
the R session exits.
...
- Pre-compute results in `analysis/scripts/` and load them here instead of
  recomputing inline
...
`make check-renv` is a pure-shell check: it scans the R files for package
usage, confirms each package is listed in `DESCRIPTION`, confirms it is
locked in `renv.lock`, and pulls in anything missing from CRAN.
...
Once rendering is clean and tests pass, flip `draft: false` in the YAML and
commit:
...
*Questions, suggestions, or spotted errors are
welcome.*
...
[ ] Analysis pipeline runs end-to-end from clean checkout
    [ ] make docker-build && make docker-render-qmd succeeds
